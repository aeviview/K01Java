...
/*
 [싱글톤 디자인 패턴]
 	...
 	- 작성방법
 		...
 		2. 외부클래스에서 생성자를 접근할 수 없으므로 new를 통해 객체생성은 불가능해진다.
...
class SingleTone
{
	...
	/*
	 JVM(자바가상머신)에 의해 프로그램이 시작될 때 정적변수는
	 미리 메소드영역에 생성되어 사용할 준비를 마치게 된다.
	 */
	private static SingleTone single = new SingleTone();
	
	/*
	 생성자를 private로 선언하면 클래스 외부에서는 호출할 수 없으므로
	 new 연산자를 통해서는 객체를 생성할 수 없게 된다.
	 */
	private SingleTone() //기본생성자
	{
	}	
...
public class E03SingleToneDesignPattern
{
	public static void main(String[] args)
	{
		...
		System.out.println(String.format("st3의주소:%s", st2));
