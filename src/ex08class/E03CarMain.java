package ex08class;
	...
	class Car
	{
		...
		/*
		 [소유주]
		 	- Human 클래스를 기반으로 하는 멤버변수로 초기화를 위해
		 	이름, 나이, 에너지 세 가지의 값이 필요함.
		...
		/*
		[initialize] 
		  객체의 초기화를 담당하는 메소드로 아래는 항상 같은 값으로만 초기화된다.
		 */
		void initialize()
		{
			carModel = "람보르기니";
		...
		void initialize(String model, int year, String name, int age, int energy)
		{
			...
			carYear = year;
			/*
			 또 다른 객체가 멤버변수로 정의되었으므로,
			 객체를 생성한 후 각각의 멤버변수를 초기화해야 한다.
			 */
			owner = new Human();
			owner.name = name;
			owner.age = age;
	...
	public class E03CarMain {
		...
		/*
		 [에러발생] : 자동차 소유주를 표현하는 Human객체가 초기화되지 않은 상태에서
		 			출력을 시도하여 런타임 에러가 발생한다.
		 */
		
		System.out.println("[초기화 메소드 호출 후]");
		car1.initialize(); //initialize를 호출해서 초기화 한다.
		car1.drive();
		car1.showCarInfo();
		
		
		//자동차2 객체 생성 - 객체 생성 후 멤버변수에 직접 접근하여 초기화 한다.
		Car car2 = new Car();
		...
		// 자동차3 객체  생성 - 매개변수가 있는 초기화 메소드를 이용하여 초기화 한다. (권장)
		Car car3 = new Car();
