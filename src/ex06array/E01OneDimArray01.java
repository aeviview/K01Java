...
public class E01OneDimArray01 {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 그래서 배열명(배열변수)은 주소값(참조값)을 할당받게 되고,
		 해당 주소를 통해 배열에 접근한다.
		 */
		
		int[] numArr = new int[3]; //크기가 10인 int형 배열의 선언 및 생성
		numArr[0] = 10; //인덱스를 이용한 배열의 초기화
		numArr[1] = 20;
		numArr[2] = 30;
		
		System.out.println("배열명(numArr)="+ numArr);
		// 이상하게 나오는 숫자&영어 -> Heap 공간의 참조값(I@15db9742)
		...
		/*
		 ...
		 <배열명.length>
		  - 해당 배열의 크기를 반환한다.
		  - 배열의 크기가 변경되는 경우에 유연하게 사용할 수 있다.
		 */
		...
		for(int i=0 ; i<40 ; i++) // 위의 length로 범위 정하는 것이 더 효율적이다
		{
		...
		/*
		 <객체배열>
		  - 기본자료형을 저장하는 배열이 아니라,
		  	객체의 참조값을 저장할 용도로 생성하는 배열이다. 사용방법은 동일하다.
		 */
		...
		System.out.println("방법1 : 배열이 선언 이후 초기화"); //데이터가 많은 경우 사용
		...
		/*
		 ...
		 자바 컴파일러는 초기값의 갯수를 통해 크기를 결정할 수 있으므로
		 이런 경우 크기는 생략하도록 약속되어 있다.
		 */
		...
		System.out.println("방법3 : 초기값만으로 선언"); //데이터가 별로 없는 경우 사용
		int[] initArr3 = {11, 13, 17, 19, 23};
		int arrSum = 0;
		for(int x=0 ; x<initArr3.length ; x++) // 0 ~ 4까지 인덱스
		{
			arrSum += initArr3[x];
		}
