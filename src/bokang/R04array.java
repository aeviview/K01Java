...
public class R04array 
{
	...
	public static void randomCreate(int[] lottoNum)
	{
		...
		//중복있으면 다시 난수 6개 생성
		//(몇 번 만들어야 할 지 모르니까 while문으로 무한루프 만드는게 좋다)
	}
	...
	public static boolean arrayExistElements(int[] arr)
	{
		...
		for(int i=0 ; i<arr.length ; i++)
		{
			for(int j=0 ; j<arr.length ; j++)
			{
				...
				{
					System.out.println("같은 요소끼리 비교 불가");
				}
			}
		}
		return isDup;
	...
	중복이 없는 경우에는 true, 중복이 있는 경우에는 false를 반환한다.
	...
	*/
	...
	//매개변수로 배열변수를 전달하고, boolean 값을 전달 받음
	...
	if(isExist2==true)
		System.out.println("arr2 : 중복없음");
	else
		System.out.println("arr2 : 중복있음");
	
	
	
	
	/*
	연습문제] 중복되지 않는 난수 6개를 생성하시오.
	생성한 난수는 아래 배열에 채워 넣으시오. (첫번째 시나리오로 중복 확인 검사하자!)
	 */
