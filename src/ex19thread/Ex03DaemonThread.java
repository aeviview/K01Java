...
[독립쓰레드(Non Daemon Thread)]
	- 메인쓰레드와 working쓰레드(개발자가 정의한 쓰레드)를 일컫는 것으로
		메인쓰레드가 종료되어도 계속 실행되며 Dead 상태가 되어야 실행이 종료된다.
	- 해당 쓰레드는 main()메소드가 종료된 이후에도 실행된다.
*/
class YesThread extends Thread
{
	...
	void threadMethod() throws InterruptedException
	{
		for(int i=1 ; i<=10; i++)
		{
			...
			//해당클래스의 이름과 변수 i를 출력한다.
		
		sleep(2000); //2초에 한 번 씩 실행된다.
		}
	}
...
class DaemonThread extends Thread
{
	...
	public void run()
	{
		while(true)
		{
			...
			{
				System.out.println("자동저장 시 오류발생함 ㅠㅠ");
			}
...
public class Ex03DaemonThread 
{
	...
	public static void main(String[] args) 
	{
		...
		nt2.notThreadMethod();
		//1번 객체가 종료되면 2번 객체가 실행되는 일반적인 형태로 실행된다.

		
		//독립쓰레드 생성 및 실행
		YesThread yt1 = new YesThread("1st 쓰레드"); //인자가 있는 생성자 (생성자를 통해 이름 부여)
		//yt1.setName("첫번째쓰레드"); 쓰레드의 이름을 지정하는 두 번째 방법(setName());
		yt1.start();
		
		YesThread yt2 = new YesThread(); //인자가 없는 생성자 (이름이 자동으로 부여)
		yt2.setPriority(Thread.MAX_PRIORITY); //우선순위를 10으로 지정(MAX니까)
		yt2.start();
		...
		System.out.println("현재 활성화 된 상태의 쓰레드 수" + Thread.currentThread().getPriority());
