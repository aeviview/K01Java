...
/*
 ...
 1) Predicate : 매개변수 있고, 리턴은 boolean값
 2) Consumer : 매개변수 있고, 리턴값 없음
 3) Supplier : 매개변수 없고, 리턴값 있음
 4) Function : 매개변수 있고, 리턴값 있음
 */
public class Ex06Define1Predicate 
{
	...
	public static void main(String[] args) 
	{
		/*
		 [Predicate]
		 	- 사전적의미 : ~에 근거를 두다
		 	- 매개변수와 boolean 리턴값이 있는 test()계열의 추상메소드를 가지고 있다.
		...
		//전달된 객체의 성별이 "남"인지 판단하여 boolean 값을 반환하는 람다식을 정의함.
		...
		/*
		 ...
		 이와 같은 람다식을 간소화하면 아래와 같이 쓸 수 있다.
		 	- 매개변수의 타입을 유추할 수 있어 Human을 생략하고
		 	- 실행문장이 하나라서 {}을 생략하였다.
		 */
	...
	public static int sum(Predicate<Integer> p, List<Integer> list)
	{
		...
		for(int a : list)
		{
			if(p.test(a)) //test()를 통해 홀/짝 여부를 판단한다.
			{
				sum += a;
			}
		}
	...
	//매개변수로 성별을 판단하는 람다식과 List컬렉션을 전달받아 평균값을 반환하는 메소드
	public static double avg(Predicate<Human> ph, List<Human> li)
	{
			...
			for(Human h : li) //li컬렉션의 크기만큼 반복
			{
				...
				//첫번째 호출에서는 남자일 때 true를 반환하므로, 남자의 인원수와 점수가 카운트 된다.
				if(ph.test(h)==true)
				{
					pCount++; //인원 수 증가
