...
public class Ex06Define2Supplier 
{
	...
	//난수를 생성하여 반환하는 람다식을 매개변수로 받아 cnt개의 난수를 반환한다.
	public static List<Integer> makeRandomNum(Supplier<Integer> s, int cnt)
	{
		List<Integer> li = new ArrayList<Integer>();
		//cnt크기만큼 반복하면서 난수를 생성 후 List에 저장한다.
		for(int i=1 ; i<=cnt ; i++)
		{
			li.add(s.get());
		}
	...
	/*
	 Supplier<T>
	 	- 공급자란 의미를 가지고 있고 매개변수는 없으나 리턴값이 있는 get() 메소드가 정의된다.
	...
	public static void main(String[] args) 
	{
		...
		//위의 get()메소드를 오버라이딩 하여 0~99사이의 난수를 반환하는 람다식을 정의한다.
		...
		//10개의 난수를 생성해서 List컬렉션으로 반환받는다.
		List<Integer> list = makeRandomNum(sup, 10);
		System.out.println("### print문으로 출력1");
		System.out.println(list); //단순히 컬렉션을 출력만 하는 용도로 사용한다.
