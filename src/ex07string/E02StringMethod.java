...
public class E02StringMethod {
	...
	public static void main(String[] args) {
		...
		/*
		 2] charAt(인덱스)
		  - 문자열에서 특징 index에 해당하는 문자 하나를 반환한다.
		  - index는 0부터 시작한다.
		 */
		...
		/*
		 [시나리오] 주민등록번호를 이용하여 성별을 판단하는 프로그램을 만들어라
		 190419-3000000 => 남자
		 190419-4000000 => 여자
		 */
		...
		/*
		 6] contains()
		  - 문자열에 특정 문자열이 포함되어 있으면 true를 반환한다.
		 */
		 ...
		 */
		//String email = "hong@ikosmo.co.kr";  //true 반환
		String email = "not@naver"; // 이메일 형식이 아니다 false 반환
		...
		/*
		 10] indexOf()
		  ...
		  - 만약 찾는 문자열이 없다면 -1를 반환한다.
		 */
		System.out.println(str1.indexOf("ava")); //12
		System.out.println(str1.indexOf("j"));   //11
		System.out.println(email.indexOf("@")!=-1 ? "이메일형식 맞음" : "이메일형식 아님");
		
		/*
		 [시나리오] 주민등록번호를 indexOf를 사용하여 성별을 구분하는 프로그램을 작성하시오.
		 */
		// '-'(하이픈)의 위치를 찾아 +1하면 성별에 해당하는 문자를 얻을 수 있다.
		...
		/*
		 11] lastIndexOf()
		  - 사용법은 indexOf()와 동일하나 뒤에서부터 문자열을 찾아서 인덱스를 반환한다.
		 */
		...
		/*
		 14] substring()
		  ...
		  - 사용 1) substring(index) -> 이경우 index부터 문자열의 끝까지 잘라서 반환함.
		  - 사용 2) substring(index1, index2) -> 인덱스에 포함되는 부분만 잘라서 반환함.
		 */
		 ...
		 */
		String filename = "my.file.imaes.jpg";
