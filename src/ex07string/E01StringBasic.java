	...
	public static void main(String[] args) {
		...
		/*
		 String 클래스의 객체 생성 방법(1)
		 	- new 연산자를 이용한다.
		 	- new 연산자를 사용하면 동일한 문자열이라도 항상 새로운 메모리를 할당받는다.
		 */
		...
		/*
		 ...
		 아래는 단순한 참조값(주소값)에 비교를 진행한다.
		 - 객체는 Heap에 만들어지게 되고 그 주소값(번지수)을 말한다.
		 */
		if(str1==str2)
		{
			System.out.println("str1과 str2는 참조 주소 같음");
		}
		else
		{
			System.out.println("str1과 str2는 참조 주소 다름");
		}
		
		/*
		 epuals()메소드
		 	- Object 클래스로부터 상속받은 메소드로 실제 저장된
		...
		/*
		 자바에서 문자열 객체를 생성할 때
		 new를 사용하지 않고 "를 사용하면 동일한 문자열인 경우에
		 메모리를 하나만 사용하여 공유하게 된다.
		 */
		if(str3==str4)
		{
