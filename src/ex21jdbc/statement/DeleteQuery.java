...
public class DeleteQuery 
{
	...
	private void execute()
	{
		...
		{
			/*
			 test99 삭제시 입력되지 않은 데이터이므로 삭제되지 않는다.
			 데이터가 없는 경우 예외는 발생하지 않는다.
			 ko1 삭제시 자식레코드가 있으므로 제약조건 위배로 삭제되지 않는다!
			 */
			stmt = con.createStatement();
			String query = "DELETE FROM member WHERE id = 'test4'"; //이걸 통해서 삭제한다!
