...
public class E05LocalDateTimeO
{
	public static void main(String[] args)
	{
		...
		//가장 가까운 요일(N번째 요일 등을 간단한 코드로 가져오기)
		...
		LocalDate xmas = LocalDate.of(LocalDate.now().getDayOfYear(),12, 25);
		Period left = Period.between(nowDate,  xmas);
		System.out.println("X-mas까지 앞으로 " + left.getMonths() + "개월 "+ left.getDays() + "일");
		
		Instant end = Instant.now();
		Duration duration = Duration.between(start,  end);
		System.out.println("클래스 실행 시간 확인(두 시간의 차) : "+ duration.toMillis());
