...
/*
 [equals()메소드]
 	- 자바에서 인스턴스의 내용비교를 위해 비교연산자(==)를 사용하는 것은
 		단순히 참조값에 대한 비교이고, 실제 내용에 대한 비교는 이루어지지 않는다.
 	- 인스턴스간의 내용비교를 위해서는 equals()메소드를 각 클래스에 맞게 오버라이딩하여 사용한다.
...
class IntNumber
{
	...
	//equals()메소드가 어떻게 인스턴스간의 내용을 비교하는지 설명하기 위한 코드로 구성된다.
	public boolean myEquals(IntNumber numObj)
	{
		if(this.num==numObj.num) //각 멤버변수의 값을 비교해서
		{
			return true; //같을 때는 true 반환
		}
		else
		{
			return false; //다를 때는 false 반환
		}
	}
}
