...
public class E01JungsuType {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 - byte, short형은 게임캐릭터의 움직임이나 음원정보를 저장할 때 사용한다.
		 */
		...
		/*
		 ...
		 - 큰 자료형과 작은 자료형을 연산하면 큰 자료형으로 자동형변환된다.
		  (데이터의 손실이 있을 수 있기 때문)
		 - 단, byte와 short는 예외적인 상황으로 생각한다.(int형에 최적화된 CPU때문에)
		 */
		...
		/*
		 자바컴파일러(CPU)는 정수형을 기본적으로 int형으로 간주하기 때문에
		 위 정수를 메모리에 올리는 순간 에러가 발생한다.
		 이때는 접미사 (L혹은l)를 붙여서 int형 자료가 아닌 long형 자료임을 알려줘야 한다.
