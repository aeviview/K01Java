	...
	public static void main(String[] args) {
		...
		/*
		 char형 : 하나의 문자를 저장할 수 있는 자료형으로 값을 저장할 때
		 		...
		 		만약 "(Double quotation)으로 감싸면
		 		문자열로 인식하므로 String형으로 선언해야 한다.
		 */
		...
		char ch2 = 'A';
		int num1 = 2;
		int num2 = ch2 + num1; // 65 + 2 = 67 =>int가 char보다 크기 때문에
		...
		/*
		 && : 논리And 연산으로 양쪽 모두 참일 때 참을 반환(선택)한다.
		 	나머지는 거짓을 반환한다.
		 || : 논리Or 연산으로 둘 중 하나만 참이면 참을 반환(선택)한다.
			둘 다 거짓일때만 거짓을 반환한다.
		 */
		
		boolean bn3 = bn1 && bn2;
		System.out.println("bn3(And)="+ bn3); //false반환
		bn3 = bn1 || bn2; // || : shift + 역슬러쉬
