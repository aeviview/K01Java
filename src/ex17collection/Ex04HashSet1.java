...
/*
 [HashSet 컬렉션] : set계열의 인터페이스를 구현한 컬렉션이다.
 	- 객체가 순서 없이 저장된다.
...
public class Ex04HashSet1 
{
	...
	public static void main(String[] args) 
	{
		...
		int number = 100; //int형이 아니라 Integer객체로 Boxing처리 후 저장된다.
		Teacher teacher = new Teacher("김봉두", 55, "체육");
		
		
		//[3.객체저장] : add()를 통해 객체가 정상적으로 저장되면 true를 반환한다.
		...
		//[5-1.기본 클래스형 객체 중복저장] : 기본클래스의 경우에는 별도의 오버라이딩 처리 없이 중복이 제거된다.
		//							아래의 경우 false 반환
		...
		//[5-2. 개발자가 정의한 객체 중복저장] : 직접 정의한 클래스인 경우 hashCode, equals 메소드를 오버라이딩
		//								하지 않으면 중복저장이 허용된다. 아래의 경우 true 반환
		...
		//[8.삭제] : Set은 index가 없으므로 참조값으로만 삭제가 가능하다.
