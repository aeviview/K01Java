...
사용자로부터 이름을 입력받아 그 이름으로 검색해서 인덱스 위치(indexOf사용)를 알아내자.
그리고 해당 인덱스로 그 객체를 삭제하고, 삭제된 객체의 정보(이름,나이,학번)를 출력하여라.
...
class Student
{
	...
	//source > construct using field > generate
	...
	//source > getters & setters > select all
...
public class QuArrayList 
{
	...
	public static void main(String[] args) 
	{
		...
		//2.확장for문으로 컬렉션 전체를 접근
		int index = -1; //배열(컬렉션)의 인덱스는항상 0부터 시작이므로 초기값은 -1로 설정한다.	
		for(Student st : list)
			{
				if(searchName.equals(st.getName())) //객체의 참조값을 통해 List내의 index 값을 찾음
				{
					index = list.indexOf(st); //해당 객체를 찾았으면 즉시 루프 탈출
					break;
				}
			}
		...
		//4.전체정보 출력
		System.err.println("검색 후 전체 정보 출력");
		for(Student st : list)
		{
