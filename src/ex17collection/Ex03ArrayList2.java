...
public class Ex03ArrayList2 
{
	...
	{
		/*
		아래 두 개의 컬렉션은 내부적인 자료 구조만 다르고, 구현방법은 동일하다.
		컬렉션 생성 시 Student 형의 타입매개변수를 사용한다.
		*/
		...
		list2.add(st3);
		list2.add(st4); //인덱스 3
		list2.add(st2); //객체의 중복저장(List 컬렉션은 허용된다)
		...
		*/
		...
		while(it2.hasNext())
		{
			...
			//위와 동일한 문장이다. next()를 통해 객체를 가져온 후 showInfo()로 객체의 내용을 출력한다.
			it2.next().showInfo();
		}
		
		
		//List 계열의 컬렉션은 인덱스로 접근 가능함
		System.out.println("[일반 for문 사용]");
		for(int i=0 ; i<list2.size() ; i++)
		{
			list2.get(i).showInfo(); //인덱스로 접근시 get()메소드를 사용한다.
		}
		...
		//인스턴스 참조값을 통한 삭제 : 삭제 성공 시 boolean 값이 반환된다.
