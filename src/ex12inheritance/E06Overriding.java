...
오버라이딩(Overriding)
	...
	멤버메소드와 완전히 동일한 모양으로 재정의하는것을 말한다.
...
class BaseSpeaker extends Speaker {
	...
	public void showState() {
		...
		특히, 오버라이딩 된 메소드를 호출할때 super가 없으면 
		재귀함수가 되므로 반드시 필요하다. 
		 */
		super.showState();
...
public class E06Overriding {
	public static void main(String[] args) {
		...
		호출할 수 없다. setBase()는 자식클래스에 정의된 멤버메소드이기 때문이다.
		단, 오버라이딩 처리된 메소드가 있는경우 하위클래스의 메소드가 호출되게된다.  
		 */
