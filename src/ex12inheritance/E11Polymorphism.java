...
/*
 다형성(Polimorphism)
 	...
 	- 부모의 참조변수로 자식 객체를 참조
 		...
 		2. 자식에서 오버라이딩한 메소드가 우선 호출
 		3. 자식에서 새롭게 정의한 멤버에는 접근 불가
 		
 	- 같은 타입의 클래스로 객체를 참조
 		: 객체 전체를 접근할 수 있다. 즉, 클래스의 일반적인 규칙이 적용된다.
 */
class Parent
{
	//멤버변수
...
class Child extends Parent
{
	...
	//오버로딩된 메소드(새롭게 확장된 메소드)
	void parentMethod(int childMember)
	{
		this.childMember = childMember;
		System.out.println("Overriding : 자식에서 확장한 메소드"
				 + " : parentMethod(param1)");
	}
...
public class E11Polymorphism {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 따라서 모든 객체(인스턴스)는 Object 참조변수로 참조 가능하다.
		 */
