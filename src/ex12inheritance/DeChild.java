...
public class DeChild extends DeParent {
	...
	/*
	오버라이딩 아님
	부모클래스에서 private 멤버메소드로 선언하였으므로
	자식에서는 보이지 않아 오버라이딩의 대상이 될수없다. 
	...
	public void printParent() {
		...
		- super.멤버메소드 : 부모클래스에 정의된 멤버메소드 혹은
			멤버변수를 접근할때 사용.
		 */
		super.printParent();
	...
	/*
	static(정적) 메소드는 오버라이딩의 대상이 될수없다.
	클래스 외부(메소드영역)에 존재하므로 오버라이딩 되지 않고
	호출할때는 각 클래스명을 통해 호출해야한다. 
	 */
