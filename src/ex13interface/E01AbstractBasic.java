	...
	//자식클래스에서 확장한 멤버 변수와 메소드
{
	...
	void instanceMethod()
	{
		System.out.println("필요없다면 오버라이딩 하지 않아도 괜찮다");
	}
...
<추상클래스2>
	- 추상메소드를 포함한 클래스
	- 추상메소드를 멤버로 포함한 클래스는 반드시 abstract로 선언해야 한다.
*/
...
/*
 부모가 가진 추상메소드를 그대로 상속받게 되므로 해당클래스도 반드시 abstract로 선언해야 한다.
 단, 이 경우 객체를 생성하지 못하게 되므로 부모의 abstract메소드를 오버라이딩 하면 된다.
 */
class AbsClassChild2 extends AbsClass2
{
	...
	{
		System.out.println("부모클래스에서 오버라이딩한 메소드 없을 때는 에러");
	}
